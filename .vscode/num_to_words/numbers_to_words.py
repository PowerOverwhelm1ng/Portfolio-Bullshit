import num2words as n2w
from tkinter import *

#import the libs to convert nums to words, tkinter gui

#func to make num to words

def num_to_words():
    given_num = float(num.get())
    num_in_word = n2w.num2words(given_num)
    display.config(text=str(num_in_word).capitalize())

#What does this code do exactly>

root= Tk()
root.title("Numbers To Words")
root.geometry("650x400")

num = StringVar()

#Add in a title

title = Label(root, text="Number to Words Converter",
              fg="Blue", font=("Arial",20,"bold")).place(x=220, y=10)

#Options

formats_label = Label(root, text="Formats supported : ", fg="green", font=("Arial", 10, "bold")).place(x=100, y=70)

pos_format_label = Label(root, text="1. Positives :", fg="green", font=("Arial", 10, "bold")).place(x=200, y=90)

                         
neg_format_label = Label(root, text="2. Negatives :", fg="green", font=("Arial", 10, "bold")).place(x=200, y=110)
float_format_label = Label(root, text="3. Zeroes :", fg="green", font=("Arial", 10, "bold")).place(x=200, y=130)
zero_format_label = Label(root, text="4. Floating Points/Decimals/Fractions : ", fg="green", font=("Arial", 10, "bold")).place(x=200, y=150)


num_entry_label = Label(root, text="Enter a Number!", fg="Blue", font=("Arial", 15, "bold")).place(x=50, y=200)
num_entry = Entry(root, textvariable=num, width=30).place(x=220, y=200)

# MAKE BUTTON (Corrected placement)
btn = Button(master=root, text="Calculate this Mofo", fg="green", font=("Arial", 10, "bold"), command=num_to_words)
btn.place(x=280, y=220)  # Changed 'y' coordinate to 220

display = Label(root, text="", fg="black", font=("arial", 10, "bold"))
display.place(x=10, y=300)

photo = PhotoImage(file="C:\\Users\\tggal\\OneDrive\\Desktop\\coding bulllshit\\Portfolio Bullshit\\.vscode\\num_to_words\\number.png")

root.iconphoto(False, photo)

root.mainloop()